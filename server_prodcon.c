...
#define mode S_IRUSR|S_IWUSR|S_IRGRP|S_IWGRP|S_IROTH|S_IWOTH  //User (owner)/group member/others can access this file
...
int main(int argc, char **argv)
{
    ...
    pthread_setschedparam(pthread_self(), SCHED_RR, &my_param); //  sets the scheduling parameters associated with the scheduling policy for the process.
    pthread_getschedparam (pthread_self(), &policy, &my_param); //  retrieves the scheduling parameters for the process.

    pthread_attr_init(&lp_attr); // initialization low priority
    pthread_attr_init(&hp_attr); // iintialization high priority
    ...
    queue *fifo; //fifo named pointer
    pthread_t pro, con1,con2,con3;  // 4 threads declared
    ...
    pthread_create (&con3, &lp_attr, consumer, fifo);  // fifo pointer given to producer function
...
void echo(int connfd)
{
    ...
    rio_t rio;
    int imgfd;
    imgfd=Open("out.png",O_WRONLY|O_CREAT,mode); // if exist then open, if not then create and set it as write only.
    Rio_readinitb(&rio, connfd); // client and conne fc relation
    while((n = Rio_readlineb(&rio, buf, MAXLINE)) != 0) {
        //printf("server received %d bytes\n", n);
        Rio_writen(imgfd, buf, n); // imgfd in buffer
    }
    Close(imgfd);
    if (fork()==0)
    {
